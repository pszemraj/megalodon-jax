...
- **Attention-mask loss semantics are target-only.** A shifted target contributes when its own `attention_mask` entry is valid even if the preceding source entry is masked. Use `ignore_index` labels for stricter loss exclusion.
...
The unmasked path uses one shifted first/second-moment `lax.cumsum`, anchored at the incoming mean when history exists and at the first token-block mean otherwise. Arbitrary masks use an identity-aware associative Welford prefix, and packed inputs use the reset-aware associative monoid. Feature reductions and output normalization remain vectorized outside every prefix operation. Masked inputs are replaced before moment and normalization arithmetic, so invalid NaN/Inf payloads cannot contaminate either forward state or reverse-mode gradients. Tests use an independent NumPy/float64 paper-equation oracle; production contains no sequence-length `while` and uses no FFI, custom call, Pallas, or custom VJP.
